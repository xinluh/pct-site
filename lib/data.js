...
const API_KEY =
  ...
  "AIzaSyApgm2nFzwLrOyISptqXF9RbiHyq1Josbk"; // this is restricted to localhost, github.io/xinluh and xinlu.dev domains
const SHEET_ID = "1gqqQljQm21SKEdzWAlrzSPcoeuJ0aurLgSxLwmbFXEs";
...
export function useGarminLogData() {
  ...
  useEffect(() => {
    promiseRetry(
      (retry, number) => {
        if (number > 1) {
          console.log("getting data attempt number", number);
        }
